...
void MD_MAX72XX::begin(void) {
  ...
  _spiData = (uint8_t *) malloc(SPI_DATA_SIZE);
#endif
  PRINTS("done allocating _spiData");

#if USE_LOCAL_FONT
#if USE_INDEX_FONT
  PRINTS("allocating fontIndex");
  _fontIndex = (uint16_t *)malloc(sizeof(uint16_t) * FONT_INDEX_SIZE);
  PRINTS("fontIndex allocation done");
#else
  _fontIndex = NULL;
#endif
  setFont(NULL);
...
bool MD_MAX72XX::control(uint8_t startDev, uint8_t endDev,
    controlRequest_t mode, int value) {
  if (endDev < startDev)
    return (false);
